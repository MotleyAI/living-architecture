"""Generate mermaid diagrams in arc42 docs from the LikeC4 model.

The single parser of the constrained `.c4` authoring convention: `arch_check` imports
this module. `la-arch-diagrams` rewrites the mapped docs in place.
"""
...
def generate(root: Path) -> list[str]:
    ...
    if not isinstance(diagrams, dict):
        raise ValueError(_bad_diagrams_reason(diagrams))  # noqa: TRY004 — malformed YAML data
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="la-arch-diagrams", description=__doc__)
    ...
    try:
        ...
        print(f"la-arch-diagrams: {exc}", file=sys.stderr)
        return 1
