...
from living_architecture import arch_diagrams
...
def _root_package(index: dict) -> str:
    ...
    if not isinstance(pkg, str) or not pkg:
        raise ArchCheckError("architecture/index.yaml must set root_package to the top-level package name")
    return pkg
...
def license(*, root: Path, src: str, dst: str) -> bool:
    ...
    the edge must be covered by a declared arrow. Exposes the one import law to other tools.
    """
...
def _tag_occurrence(kind: str, rest: str, name: str, issue_key_re: re.Pattern[str]) -> tuple[bool, list[str]]:
    ...
    if kind == "target":
        if issue_key_re.fullmatch(tag_id) is None:
            return False, [
                (
                    f"enforced-tags: {name} target tag id {tag_id!r} does not match issue_key_pattern"
                    f" {issue_key_re.pattern!r}"
                )
            ]
        return True, []
...
def _check_enforced_tags(root: Path, issue_key_re: re.Pattern[str]) -> list[str]:
    findings: list[str] = []
    for path in sorted((root / "architecture").glob("*.arc42.md")):
...
def run_checks(root: Path) -> list[str]:
    ...
    findings += _check_enforced_tags(root, load_config(root).issue_key_re())
    findings += arch_diagrams.check_diagrams_fresh(root=root, model=model, views=views)
    return findings


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="la-arch-check", description=__doc__)
    ...
    except (ArchCheckError, ConfigError, OSError, yaml.YAMLError) as exc:
        print(f"arch_check: {exc}", file=sys.stderr)
        return 2
