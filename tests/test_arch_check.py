...
from living_architecture import arch_check
...
def test_root_package_is_required(tmp_path):
    ...
    with pytest.raises(arch_check.ArchCheckError, match="root_package"):
        arch_check.run_checks(root)
...
def test_main_ok(tmp_path, capsys):
    root = make_repo(tmp_path)
    assert arch_check.main(["--root", str(root)]) == 0
...
def test_main_findings(tmp_path, capsys):
    root = make_repo(tmp_path)
    append_principles(root, "3. Naked rule.\n")
    assert arch_check.main(["--root", str(root)]) == 1
...
def test_main_broken_setup(tmp_path, capsys):
    ...
    assert arch_check.main(["--root", str(root)]) == 2
    assert "nope" in capsys.readouterr().err


def test_main_without_index(tmp_path, capsys):
    assert arch_check.main(["--root", str(tmp_path)]) == 2
