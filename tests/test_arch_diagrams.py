"""arch_diagrams generator + parser, and arch_check's diagrams-fresh check."""
...
from living_architecture import arch_check, arch_diagrams
from living_architecture.arch_diagrams import FIX_CMD
...
def test_main_prints_changed_files(tmp_path, capsys):
    root = make_repo(tmp_path, regenerate=False)
    rc = arch_diagrams.main(["--root", str(root)])
...
def test_main_reports_errors_without_traceback(tmp_path, capsys):
    ...
    assert arch_diagrams.main(["--root", str(root)]) == 1
